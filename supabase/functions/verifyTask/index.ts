...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (taskData.is_complete !== true || taskData.verification_status !== 'pending') {
      ...
      return new Response(
        JSON.stringify({ error: 'Task is not marked complete or is already verified.' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!userIsAdmin && !userIsLinkedTeacher) {
      console.warn(
        `Authorization failed: User ${verifierId} is not Admin or linked Teacher for student ${studentId}.`
      );
      return new Response(
    ...
    console.log(
      `Authorization success: User ${verifierId} is ${userIsAdmin ? 'Admin' : 'Linked Teacher'} for student ${studentId}.`
    );
