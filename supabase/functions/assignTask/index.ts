...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!payload.studentId || !payload.taskTitle || payload.taskBasePoints == null || payload.taskBasePoints < 0) {
      return new Response(JSON.stringify({ error: 'Missing required fields: studentId, taskTitle, taskBasePoints (>= 0).' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!userIsAdmin) {
        ...
        const { data: callerProfile } = await supabaseAdminClient.from('profiles').select('role').eq('id', assignerId).single();
        if (callerProfile?.role === 'teacher') {
            userIsLinkedTeacher = await isTeacherLinked(supabaseAdminClient, assignerId, payload.studentId);
        }
    }

    if (!userIsAdmin && !userIsLinkedTeacher) {
         console.warn(`Authorization failed: User ${assignerId} is not Admin or linked Teacher for student ${payload.studentId}.`);
         return new Response(JSON.stringify({ error: 'Permission denied: User cannot assign tasks to this student.' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    console.log(`Authorization success: User ${assignerId} is ${userIsAdmin ? 'Admin' : 'Linked Teacher'}.`);
    ...
    if (insertError) {
      ...
      return new Response(JSON.stringify({ error: `Failed to assign task: ${insertError.message}` }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
