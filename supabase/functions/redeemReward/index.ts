...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!payload.studentId || !payload.rewardId) {
      return new Response(JSON.stringify({ error: 'Missing required fields: studentId, rewardId.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (!userIsAdmin) {
         ...
         return new Response(JSON.stringify({ error: 'Permission denied: Only Admins can perform redemption currently.' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    console.log(`Authorization success: Admin ${redeemerId} performing redemption.`);

    // 7. Call the Database RPC Function
    console.log(`Calling RPC redeem_reward_for_student for Student: ${payload.studentId}, Reward: ${payload.rewardId}, Redeemer: ${redeemerId}`);
    ...
    if (rpcError) {
      ...
      return new Response(JSON.stringify({ error: `Database error during redemption: ${rpcError.message}` }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    // The RPC function returns an object { success: boolean, message: text, new_balance: integer }
     if (!rpcResult) {
         // This case might happen if maybeSingle() finds no row, which shouldn't occur for this RPC returning a table.
          ...
          return new Response(JSON.stringify({ error: 'Redemption process failed unexpectedly.' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
     }
    ...
    if (rpcResult.success) {
      return new Response(JSON.stringify({ message: rpcResult.message, newBalance: rpcResult.new_balance }), {
