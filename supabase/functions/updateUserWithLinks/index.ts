  ...
  try {
    ...
    if (idsToDelete.length > 0) {
      ...
      if (deleteError) {
        ...
        console.log(`[syncLinkTable] Successfully deleted ${idsToDelete.length} old links from ${tableName}.`);
      }
    }
    if (idsToInsert.length > 0) {
      const rowsToInsert = idsToInsert.map(linkId => ({ student_id: studentId, [linkColumnName]: linkId }));
      ...
      if (insertError) {
        ...
        errors.push(`Failed to insert new ${linkColumnName} links (check if IDs exist).`);
          ...
          console.log(`[syncLinkTable] Successfully inserted ${idsToInsert.length} new links into ${tableName}.`);
      }
    }
  } catch (syncError) {
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method !== 'POST' && req.method !== 'PATCH' && req.method !== 'PUT') { return new Response(JSON.stringify({ error: `Method ${req.method} Not Allowed` }), { status: 405, headers: corsHeaders }); }
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) { return new Response(JSON.stringify({ error: 'Authentication required.' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); }
    const token = authHeader.replace('Bearer ', '');
    const { data: { user: callerUser }, error: userError } = await supabaseAdminClient.auth.getUser(token);
    if (userError || !callerUser) { return new Response(JSON.stringify({ error: 'Invalid or expired token.' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); }
    const callerId = callerUser.id;
    console.log('Caller User ID:', callerId);

    let requestBody: UpdateUserRequestBody;
    try { requestBody = await req.json(); } catch (jsonError) { return new Response(JSON.stringify({ error: 'Invalid request body: Must be JSON.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); }
    console.log('Received payload:', requestBody);

    if (!requestBody.userIdToUpdate || !requestBody.updates || typeof requestBody.updates !== 'object') { return new Response(JSON.stringify({ error: 'Missing required fields: userIdToUpdate, updates.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); }
    ...
    if (callerProfileError || !callerProfile) { return new Response(JSON.stringify({ error: 'Could not verify caller identity.' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); }
    callerRole = callerProfile.role;
    console.log(`Caller role: ${callerRole}`);

    const { data: targetProfile, error: targetProfileError } = await supabaseAdminClient
        .from('profiles').select('role').eq('id', userIdToUpdate).single();
    if (targetProfileError || !targetProfile) { return new Response(JSON.stringify({ error: 'Target user not found or profile inaccessible.' }), { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); }
    ...
    if (callerRole === 'admin') {
        ...
        if (teacherIsLinked) {
            isAuthorized = true;
            console.log(`Authorization granted: Teacher ${callerId} is linked to Student ${userIdToUpdate}.`);
        } else {
            console.warn(`Authorization failed: Teacher ${callerId} is not linked to Student ${userIdToUpdate}.`);
        }
    }

    if (!isAuthorized) {
         return new Response(JSON.stringify({ error: 'Permission denied: You are not authorized to update this user.' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (updates.firstName !== undefined && typeof updates.firstName === 'string') { profileUpdates.first_name = updates.firstName.trim(); hasProfileUpdates = true; }
    if (updates.lastName !== undefined && typeof updates.lastName === 'string') { profileUpdates.last_name = updates.lastName.trim(); hasProfileUpdates = true; }
    if (updates.hasOwnProperty('nickname')) { profileUpdates.nickname = updates.nickname === null ? null : updates.nickname?.trim(); hasProfileUpdates = true; }

    const allErrors: string[] = [];

    if (hasProfileUpdates) {
        console.log(`Updating profile for ${userIdToUpdate}:`, profileUpdates);
        const { error: profileUpdateError } = await supabaseAdminClient.from('profiles').update(profileUpdates).eq('id', userIdToUpdate);
        if (profileUpdateError) { allErrors.push('Failed to update profile details.'); console.error(`Error updating profile for ${userIdToUpdate}:`, profileUpdateError.message);}
        else { console.log(`Profile updated successfully for ${userIdToUpdate}.`); }
    } else { console.log(`No basic profile updates for ${userIdToUpdate}.`); }

    if (isTargetStudent && updates.instrumentIds !== undefined && (callerRole === 'admin' || callerRole === 'teacher')) {
        console.log(`Syncing instruments for student ${userIdToUpdate} (caller: ${callerRole})`);
        const { errors: instrumentErrors } = await syncLinkTable(supabaseAdminClient, 'student_instruments', userIdToUpdate, updates.instrumentIds || [], 'instrument_id');
        allErrors.push(...instrumentErrors);
    } else if (updates.instrumentIds !== undefined) {
         console.warn(`Skipping instrument sync: Target is not student OR caller (${callerRole}) not authorized.`);
    }
    ...
    if (isTargetStudent && updates.linkedTeacherIds !== undefined) {
        if (callerRole === 'admin') {
            ...
            const { errors: teacherErrors } = await syncLinkTable(supabaseAdminClient, 'student_teachers', userIdToUpdate, updates.linkedTeacherIds || [], 'teacher_id');
            allErrors.push(...teacherErrors);
        } else {
            console.warn(`Teacher link update skipped: Caller (${callerId}, role: ${callerRole}) is not an Admin.`);
        }
    }

    if (allErrors.length > 0) {
      return new Response(JSON.stringify({ error: `User update completed with errors: ${allErrors.join('; ')}` }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    return new Response(JSON.stringify(successResponse), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 200 });
    ...
    return new Response(JSON.stringify({ error: error.message || 'An unexpected error occurred.' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: statusCode });
  }
});
