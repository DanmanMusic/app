...
_(Focus: Migrate DB structure and client-side reads/simple writes, defer complex/auth-dependent writes)_
_(All items previously checked remain checked)_
...
- [ ] **Implement Authentication:**
  ...
  - [ ] Frontend: Refactor `AuthContext` to handle real Supabase sessions (JWTs, refresh tokens, user state). _(Partially done by using `setMockAuthState` with real IDs/roles, but full session management pending)._
