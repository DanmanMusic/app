...
- **User Linking:** Admin is responsible for establishing relationships via the Admin interface:
  ...
  - Linking **Student** accounts to **Parent** access (mechanism detailed below). A single Parent device/session can be linked to multiple Pupils.
...
- **Authentication (Non-Admin Users - Teacher, Student, Parent):**
  ...
  - Parents install the app, select "Login", and scan the "Parent Link QR" generated by an **Admin** for their child. This logs them in and links their session to that Student. If they have multiple children, they scan a Parent Link QR for *each* child they want to access on that device.
