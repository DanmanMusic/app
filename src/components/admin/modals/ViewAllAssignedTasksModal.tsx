...
const modalStyles = StyleSheet.create({
    ...
    footer: { width: '100%', padding: 15, borderTopWidth: 1, borderTopColor: colors.borderPrimary, backgroundColor: colors.backgroundPrimary, borderBottomLeftRadius: 10, borderBottomRightRadius: 10 },
});
...
const AssignedTaskDetailItem = ({
    ...
    return (
        <View style={adminSharedStyles.taskItem}>
            ...
            <Text style={appSharedStyles.itemDetailText}>Student: {student ? getUserDisplayName(student) : item.studentId}</Text>
            <Text style={adminSharedStyles.taskItemStatus}>Status: {getStatusText()}</Text>
            <Text style={appSharedStyles.itemDetailText}>Assigned: {new Date(item.assignedDate).toLocaleDateString()} by {assigner ? getUserDisplayName(assigner) : item.assignedById}</Text>
            ...
            {item.actualPointsAwarded !== undefined && item.verificationStatus !== 'pending' && (
                <Text style={adminSharedStyles.taskItemTickets}>Awarded: {item.actualPointsAwarded ?? 0} Tickets</Text>
            )}
            ...
            <View style={adminSharedStyles.assignedTaskActions}>
                 ...
                 {onDeleteAssignment && allowDelete && (
                    <Button title="Remove (Mock)" onPress={() => onDeleteAssignment(item.id)} color={colors.danger} />
                 )}
             </View>
        </View>
    );
  ...
  onDeleteAssignment,
    ...
    const filteredTasks = useMemo(() => {
        switch (filterStatus) {
            ...
            case 'completed':
                return sortedTasks.filter(task => task.isComplete && task.verificationStatus && task.verificationStatus !== 'pending');
            case 'all':
            default:
                return sortedTasks;
        }
