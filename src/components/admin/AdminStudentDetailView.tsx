...
import { View, Text, StyleSheet, ScrollView, Button, Alert, FlatList, Platform, SafeAreaView } from 'react-native';

// Import types
import { TaskLibraryItem } from '../../mocks/mockTaskLibrary';
import { AssignedTask, TaskVerificationStatus } from '../../mocks/mockAssignedTasks';
import { Instrument } from '../../mocks/mockInstruments';
import { PupilViewProps } from '../../views/PupilView';

// Import helpers
import { getTaskTitle, getInstrumentNames } from '../../utils/helpers';

// Reuse TicketHistoryItem component
    ...
    return (
        <SafeAreaView style={adminSharedStyles.safeArea}>
            ...
            <ScrollView style={adminSharedStyles.container}>
                ...
                <Text style={adminSharedStyles.detailText}>Instrument(s): {getInstrumentNames(user.instrumentIds, mockInstruments)}</Text>
                <Text style={[adminSharedStyles.detailText, {fontWeight: 'bold'}]}>Balance: {balance} Tickets</Text>
                ...
                {assignedTasks.length > 0 ? (
                    <FlatList
                        data={assignedTasks.sort((a, b) => new Date(b.assignedDate).getTime() - new Date(a.assignedDate).getTime())}
                        ...
                        renderItem={({ item }) => (
                            <View style={adminSharedStyles.taskItem}>
                                ...
                                {item.completedDate && <Text style={adminSharedStyles.taskItemDetail}>Completed: {new Date(item.completedDate).toLocaleDateString()}</Text>}
                                {item.verifiedDate && item.verificationStatus !== 'pending' && <Text style={adminSharedStyles.taskItemDetail}>Verified: {new Date(item.verifiedDate).toLocaleDateString()}</Text>}
                                {item.actualPointsAwarded !== undefined && item.verificationStatus !== 'pending' && (
                                    <Text style={adminSharedStyles.taskItemDetail}>Awarded: {item.actualPointsAwarded ?? 0} points</Text>
                                )}
                                ...
                                <View style={adminSharedStyles.assignedTaskActions}>
                                    ...
                                    {item.isComplete && item.verificationStatus === 'pending' && (
                                        <Button title="Verify (Mock)" onPress={() => Alert.alert("Mock Admin Verify", `Simulate admin verification of task ${item.id}`)} />
                                    )}
                                    {/* Keep (Mock) as it only alerts */}
                                    <Button title="Delete (Mock)" onPress={() => Alert.alert("Mock Admin Delete Task", `Simulate admin deleting assigned task ${item.id}`)} color="red" />
                                </View>
                            </View>
                        )}
                        scrollEnabled={false}
                        ItemSeparatorComponent={() => <View style={{ height: 8 }} />}
                        ListEmptyComponent={() => <Text style={adminSharedStyles.emptyListText}>No tasks assigned.</Text>}
                    />
                ...
                <FlatList
                    ...
                    ListEmptyComponent={() => <Text style={adminSharedStyles.emptyListText}>No history yet.</Text>}
                />
                {history.length > 5 && (
