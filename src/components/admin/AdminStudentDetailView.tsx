...
export const AdminStudentDetailView: React.FC<AdminStudentDetailViewProps> = ({
  ...
  const { data: fetchedTaskLibrary = [], isLoading: taskLibraryLoading } = useQuery<
    TaskLibraryItem[],
    Error
  >({
  ...
  const handleVerifyTaskClick = (task: AssignedTask) => {
    if (onInitiateVerification) {
      ...
      console.warn('onInitiateVerification not provided');
      alert(`Mock Verify Task ${task.taskTitle}`);
    }
  };
    ...
    <>
      <SafeAreaView style={appSharedStyles.safeArea}>
        ...
        <ScrollView style={appSharedStyles.container}>
          ...
          <Text style={appSharedStyles.itemDetailText}>
            {' '}
            Instrument(s): {getInstrumentNames(user.instrumentIds, fetchedInstruments)}{' '}
          </Text>
