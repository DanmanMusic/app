...
export const AdminAnnouncementsSection: React.FC<AdminAnnouncementsSectionProps> = ({
    ...
    return (
        <View>
            <Text style={adminSharedStyles.sectionTitle}>Announcements & Challenges ({announcements.length})</Text>
                         ...
                         <AnnouncementItemPupil item={item} />
                          <View style={adminSharedStyles.itemActions}>
                              ...
                              <Button title="Edit (Mock)" onPress={() => handleEditAnnouncement(item.id)} />
                               {/* Keep (Mock) as it only alerts */}
                              <Button title="Delete (Mock)" onPress={() => handleDeleteAnnouncement(item.id)} color="red" />
                          </View>
                     </View>
                )}
                scrollEnabled={false}
                ItemSeparatorComponent={() => <View style={{ height: 10 }} />}
                ListEmptyComponent={() => <Text style={adminSharedStyles.emptyListText}>No announcements found.</Text>}
            />
        </View>
    );
};