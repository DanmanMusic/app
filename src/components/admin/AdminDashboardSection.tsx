    ...
    <View>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Text style={appSharedStyles.itemDetailText}>Total Students: {isErrorCounts ? 'Error' : studentCount}</Text>
          <Text style={appSharedStyles.itemDetailText}>Total Teachers: {isErrorCounts ? 'Error' : teacherCount}</Text>
          <Text style={appSharedStyles.itemDetailText}>Total Parents: {isErrorCounts ? 'Error' : parentCount}</Text>
          <Text style={appSharedStyles.itemDetailText}>
            Tasks Pending Verification: {isErrorTaskStats ? 'Error' : pendingVerificationsCount}
          </Text>
           {isErrorCounts && <Text style={styles.errorText}>Failed to load user counts: {errorCounts?.message}</Text>}
           {isErrorTaskStats && <Text style={styles.errorText}>Failed to load task stats: {errorTaskStats?.message}</Text>}
