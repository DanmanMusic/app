...
const AvailableTasks: React.FC<AvailableTasksProps> = ({ studentId }) => {
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Invalidate both self-assignable and regular task lists
            ...
            >
              ...
              >
                <View style={commonSharedStyles.flex1}>
                  ...
                  {task.reference_url && (
                    <TouchableOpacity onPress={() => handleOpenUrl(task.reference_url)}>
                      <Text style={commonSharedStyles.baseSecondaryText}>
                        Reference: <Text style={commonSharedStyles.linkText}>View Link</Text>
                      </Text>
                    </TouchableOpacity>
                  )}
                  {task.attachment_path && (
                    <TouchableOpacity onPress={() => handleViewAttachment(task.attachment_path)}>
                      <Text style={commonSharedStyles.baseSecondaryText}>
                        Attachment: <Text style={commonSharedStyles.linkText}>View Attachment</Text>
                      </Text>
                    </TouchableOpacity>
                  )}
                </View>
                <View>
                  <Button
                    ...
                    onPress={() => assignMutation.mutate(task.id)}
