...
import PaginationControls from '../components/admin/PaginationControls'; // Use existing control
import { adminSharedStyles } from '../components/admin/adminSharedStyles'; // Keep styles for now

// Hooks
import { usePaginatedStudentTasks } from '../hooks/usePaginatedStudentTasks';
import { usePaginatedStudentHistory } from '../hooks/usePaginatedStudentHistory'; // Import even if not used yet

// Types
import { AssignedTask } from '../mocks/mockAssignedTasks';
import { TaskLibraryItem } from '../mocks/mockTaskLibrary';
import { Instrument } from '../mocks/mockInstruments';
import { SimplifiedStudent } from '../types/dataTypes'; // Keep SimplifiedStudent
import { User } from '../types/userTypes'; // Keep User
...
// --- PendingVerificationItem Component ---
...
const StudentListItem = ({
    ...
    student: SimplifiedStudent; // Expects isActive flag now
    ...
    <View style={[appSharedStyles.itemContainer, !student.isActive ? styles.inactiveItemStyle : {}]}>
      ...
      <View style={styles.studentActions}>
        <Button title="View Profile" onPress={() => onViewProfile(student.id)} />
         {student.isActive && (
            <Button title="Assign Task" onPress={() => onAssignTask(student.id)} />
         )}
      </View>
    </View>
  );

// --- TaskLibraryItemTeacher Component ---
...
export const TeacherView: React.FC<TeacherViewProps> = ({ onInitiateVerificationModal }) => {
  ...
  // --- Memos ---
   const allStudents = useMemo( () => Object.values(currentMockUsers) .filter(u => u.role === 'student') .map(student => ({ id: student.id, name: getUserDisplayName(student), instrumentIds: student.instrumentIds, balance: ticketBalances[student.id] || 0, isActive: student.status === 'active', })) .sort((a, b) => a.name.localeCompare(b.name)), [currentMockUsers, ticketBalances] );
   const studentsLinkedToTeacher = useMemo( () => teacherUser ? allStudents.filter(student => currentMockUsers[student.id]?.linkedTeacherIds?.includes(teacherUser.id) ) : [], [allStudents, teacherUser, currentMockUsers] );
   const pendingVerifications = useMemo( () => teacherUser ? assignedTasks.filter( task => task.isComplete && task.verificationStatus === 'pending' && currentMockUsers[task.studentId]?.linkedTeacherIds?.includes(teacherUser.id) && currentMockUsers[task.studentId]?.status === 'active' ) : [], [assignedTasks, teacherUser, currentMockUsers] );

  const viewingStudentUser: User | null = useMemo(() => (viewingStudentId ? currentMockUsers[viewingStudentId] : null), [viewingStudentId, currentMockUsers]);
  const viewingStudentBalance = useMemo(() => (viewingStudentId ? ticketBalances[viewingStudentId] || 0 : 0), [viewingStudentId, ticketBalances]);

  // --- Use Pagination Hooks for the Detail View ---
  const {
  ...
  const handleAssignTaskConfirm = ( studentId: string, taskTitle: string, taskDescription: string, taskBasePoints: number ) => { if (teacherUser) { simulateAssignTask(studentId, taskTitle, taskDescription, taskBasePoints, teacherUser.id); } else { console.error("Teacher user not found"); alert("Error: Could not assign task."); } };
  const handleRemoveAssignedTask = (assignmentId: string) => { simulateDeleteAssignedTask(assignmentId); };


  if (!teacherUser) { return ( <SafeAreaView style={appSharedStyles.safeArea}><Text>Loading Teacher Data...</Text></SafeAreaView> ); }
    ...
    return (
      <SafeAreaView style={appSharedStyles.safeArea}>
        ...
        <ScrollView style={appSharedStyles.container}>
          ...
          <Text style={appSharedStyles.itemDetailText}>Status: <Text style={{fontWeight: 'bold', color: isStudentActive ? colors.success : colors.secondary}}>{viewingStudentUser.status}</Text></Text>
          <Text style={appSharedStyles.itemDetailText}> Instrument(s): {getInstrumentNames(viewingStudentUser.instrumentIds, mockInstruments)} </Text>
          <Text style={[appSharedStyles.itemDetailText, appSharedStyles.textGold]}> Balance: {viewingStudentBalance} Tickets </Text>

          {/* Assign Task Button (only if active) */}
          <View style={{ marginTop: 20, marginBottom: 20, alignItems: 'flex-start' }}>
            <Button title={`Assign Task to ${studentDisplayName}`} onPress={() => handleInitiateAssignTaskForStudent(viewingStudentUser.id)} disabled={!isStudentActive} />
          </View>

          {/* Assigned Tasks List - Use Paginated Data */}
          <Text style={appSharedStyles.sectionTitle}> Assigned Tasks ({totalTasksCount}) </Text>
          {totalTasksCount > 0 ? (
            <FlatList
              data={paginatedTasks} // Use paginated data
                    ...
                    return (
                      <View style={adminSharedStyles.taskItem}>
                        ...
                        <View style={adminSharedStyles.assignedTaskActions}>
                          {allowVerify && (
                            <Button title="Verify Task" onPress={() => onInitiateVerificationModal(item)} />
                          )}
                          {allowDelete && (
                            <Button title="Remove" onPress={() => handleRemoveAssignedTask(item.id)} color={colors.danger} />
                          )}
                        </View>
                      </View>
                    );
              }}
              scrollEnabled={false} // List doesn't scroll, page does
           ...
           {totalHistoryCount > 0 ? (
             <Text style={{ fontStyle: 'italic', color: colors.textLight, marginBottom: 10 }}>(Pagination controls would go here for history)</Text>
           ) : (
             <Text style={appSharedStyles.emptyListText}>No history found.</Text>
           )}


          {/* Mock buttons for other actions */}
         ...
         {/* Assign Task Modal */}
         <AssignTaskModal visible={isAssignTaskModalVisible} onClose={handleAssignTaskModalClose} allStudents={studentsLinkedToTeacher.filter(s => s.isActive)} taskLibrary={taskLibrary} onAssignTask={handleAssignTaskConfirm} preselectedStudentId={assignTaskTargetStudentId} />
      </SafeAreaView>
    );
  }

  // --- Main Teacher Dashboard Rendering ---
  ...
  return (
    <SafeAreaView style={appSharedStyles.safeArea}>
            ...
            <Text style={appSharedStyles.sectionTitle}> Pending Verifications ({pendingVerifications.length}) </Text>
             {pendingVerifications.length > 0 ? (
                <FlatList
                    ...
                    keyExtractor={item => item.id}
                    renderItem={({ item }) => { const studentInfo = allStudents.find(s => s.id === item.studentId); return ( <PendingVerificationItem task={item} studentName={studentInfo?.name || 'Unknown Student'} onInitiateVerification={onInitiateVerificationModal} /> ); }}
                    scrollEnabled={false}
            ...
            <Text style={appSharedStyles.sectionTitle}> My Students ({studentsLinkedToTeacher.length}) </Text>
             {studentsLinkedToTeacher.length > 0 ? (
                <FlatList
                    ...
                    keyExtractor={item => item.id}
                    renderItem={({ item }) => ( <StudentListItem student={item} mockInstruments={mockInstruments} onViewProfile={(id) => { setViewingStudentId(id); setViewingSection('studentProfile'); }} onAssignTask={handleInitiateAssignTaskForStudent} /> )}
                    scrollEnabled={false}
                    ItemSeparatorComponent={() => <View style={{ height: 10 }} />}
      ...
      {/* Assign Task Modal (for general use) */}
      <AssignTaskModal visible={isAssignTaskModalVisible} onClose={handleAssignTaskModalClose} allStudents={studentsLinkedToTeacher.filter(s => s.isActive)} taskLibrary={taskLibrary} onAssignTask={handleAssignTaskConfirm} preselectedStudentId={assignTaskTargetStudentId} />
...
const styles = StyleSheet.create({
    ...
    inactiveItemStyle: { // Added style for inactive student list items
