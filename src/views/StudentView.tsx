...
const RewardItemStudent = ({
  item, currentBalance, isGoal,
}: { item: RewardItem; currentBalance: number; isGoal: boolean; }) => {
...
export const StudentView: React.FC<StudentViewProps> = ({ studentIdToView }) => {  
  ...
  const studentHistory = useMemo(() =>
      targetStudentId ? allTicketHistory
        ...
        <View style={styles.contentArea}>
            ...
            {activeTab === 'dashboard' && (
                 <ScrollView>
                    ...
                    <FlatList data={studentHistory.slice(0, 5)} keyExtractor={item => `history-${item.id}`} renderItem={({ item }) => <TicketHistoryItem item={item} />} ItemSeparatorComponent={() => <View style={{ height: 5 }} />} ListEmptyComponent={() => (<Text style={appSharedStyles.emptyListText}>No history yet.</Text>)} scrollEnabled={false} contentContainerStyle={styles.listContentContainer}/>
                    {studentHistory.length > 5 && (<View style={{ alignItems: 'flex-start', marginTop: 10 }}><Button title="View Full History (Mock)" onPress={() => alert('Navigate to full history screen')} /></View>)}
            ...
            {activeTab === 'tasks' && (
                <ScrollView>
                    ...
                    <FlatList
                        ...
                        ItemSeparatorComponent={() => <View style={{ height: 10 }} />} ListEmptyComponent={() => (<Text style={appSharedStyles.emptyListText}>No active tasks assigned.</Text>)} scrollEnabled={false} contentContainerStyle={styles.listContentContainer}/>
                    {pendingVerificationTasks.length > 0 && (<>
                        ...
                        <FlatList data={pendingVerificationTasks} keyExtractor={item => `pending-task-${item.id}`} renderItem={({ item }) => (<AssignedTaskItem task={item} taskLibrary={taskLibrary} onMarkComplete={simulateMarkTaskComplete} canMark={canMarkComplete}/>)} ItemSeparatorComponent={() => <View style={{ height: 10 }} />} ListEmptyComponent={() => <Text style={appSharedStyles.emptyListText}>None</Text>} scrollEnabled={false} contentContainerStyle={styles.listContentContainer}/>
                    </>)}
                    {recentlyCompletedTasks.length > 0 && (<>
                        <Text style={appSharedStyles.sectionTitle}>Recently Completed Tasks ({recentlyCompletedTasks.length})</Text>
                        <FlatList data={recentlyCompletedTasks} keyExtractor={item => `completed-task-${item.id}`} renderItem={({ item }) => (<AssignedTaskItem task={item} taskLibrary={taskLibrary} onMarkComplete={simulateMarkTaskComplete} canMark={canMarkComplete}/>)} ItemSeparatorComponent={() => <View style={{ height: 10 }} />} ListEmptyComponent={() => <Text style={appSharedStyles.emptyListText}>None</Text>} scrollEnabled={false} contentContainerStyle={styles.listContentContainer}/>
                    </>)}
                    <View style={{ height: 30 }} />
                 </ScrollView>
            )}
