...
import {
  View, Text, StyleSheet, ScrollView, FlatList, Button, Image, ActivityIndicator, Alert
} from 'react-native';
...
export const StudentView: React.FC<StudentViewProps> = ({ studentIdToView }) => {
  ...
  const { data: user, isLoading: userLoading, isError: userError, error: userErrorMsg } = useQuery<User, Error>({
    ...
    queryFn: async () => {
      ...
      if (!userData || userData.role !== 'student') throw new Error('Fetched user is not a student');
      return userData;
    },
  ...
  const { data: balance = 0, isLoading: balanceLoading, isError: balanceError, error: balanceErrorMsg } = useQuery<number, Error>({
  ...
  const { data: rewardsCatalog = [], isLoading: rewardsLoading, isError: rewardsError, error: rewardsErrorMsg } = useQuery<RewardItem[], Error>({
    queryKey: ['rewards'],
  ...
  const { data: allAnnouncements = [], isLoading: announcementsLoading, isError: announcementsError, error: announcementsErrorMsg } = useQuery<Announcement[], Error>({
  ...
  const { data: mockInstruments = [], isLoading: instrumentsLoading, isError: instrumentsError, error: instrumentsErrorMsg } = useQuery<Instrument[], Error>({
  ...
  const { tasks: paginatedTasks, currentPage: tasksCurrentPage, totalPages: tasksTotalPages, setPage: setTasksPage, isLoading: tasksLoading, isError: tasksError, error: tasksErrorObject, totalTasksCount } = usePaginatedStudentTasks(targetStudentId);
  const { history: paginatedHistory, currentPage: historyCurrentPage, totalPages: historyTotalPages, setPage: setHistoryPage, isLoading: historyLoading, isError: historyError, error: historyErrorObject, totalItems: totalHistoryCount } = usePaginatedStudentHistory(targetStudentId);
  ...
  const markCompleteMutation = useMutation({
    ...
    onError: (error, assignmentId) => {
      ...
      Alert.alert('Error', `Failed to mark task complete: ${error instanceof Error ? error.message : 'Unknown error'}`);
    },
  });
  ...
  if (userError) {
    return (
      <SafeAreaView style={appSharedStyles.safeArea}>
        <View style={appSharedStyles.container}>
          <Text style={appSharedStyles.textDanger}>Error loading student data: {userErrorMsg?.message}</Text>
  ...
  return (
    <SafeAreaView style={appSharedStyles.safeArea}>
      <View style={appSharedStyles.container}>
        ...
        {/* Balance Display */}
        {balanceLoading ? ( <Text style={[styles.balance, appSharedStyles.textGold]}>Loading balance...</Text> )
         : balanceError ? ( <Text style={[styles.balance, appSharedStyles.textDanger]}>Error loading balance: {balanceErrorMsg?.message}</Text> )
        ...
        <View style={styles.contentArea}>
          ...
          {activeTab === 'dashboard' && (
            <ScrollView>
              ...
              {rewardsError && <Text style={appSharedStyles.textDanger}>Error loading rewards for goal.</Text>}
              {!rewardsLoading && !rewardsError && (
                goalReward ? (
                    <View style={styles.goalContainer}>
                        ...
                        {goalMet ? ( <Text style={styles.progressText}> Progress: {goalReward.cost} / {goalReward.cost} (100.0%) {balance > goalReward.cost && ` with ${balance - goalReward.cost} remaining`} </Text> )
                         : ( <Text style={styles.progressText}> Progress: {balance} / {goalReward.cost} ({clampedProgress.toFixed(1)}%) </Text> )}
                        <View style={styles.progressBarBackground}>
                            <View style={[ styles.progressBarFill, { width: `${clampedProgress}%`, backgroundColor: goalMet ? colors.success : colors.gold, }, ]}/>
                        </View>
              ...
              {historyLoading && <ActivityIndicator color={colors.primary} style={{ marginVertical: 10 }}/>}
              {historyError && <Text style={appSharedStyles.textDanger}>Error loading history: {historyErrorObject?.message}</Text>}
              ...
              {!historyLoading && !historyError && totalHistoryCount > paginatedHistory.length && (
                <View style={{ alignItems: 'flex-start', marginTop: 10 }}>
                  <Button title="View Full History" onPress={() => alert('TODO: Navigate to full history screen/tab')} />
                </View>
              )}
          ...
          {activeTab === 'tasks' && (
            <>
              {tasksLoading && <ActivityIndicator color={colors.primary} style={{ marginVertical: 10 }}/>}
              {tasksError && <Text style={appSharedStyles.textDanger}>Error loading tasks: {tasksErrorObject?.message}</Text>}
          ...
          {activeTab === 'rewards' && (
             <>
              {rewardsLoading && <ActivityIndicator color={colors.primary} style={{ marginVertical: 10 }}/>}
              {rewardsError && <Text style={[appSharedStyles.textDanger, {textAlign: 'center', marginTop: 10}]}>Error loading rewards: {rewardsErrorMsg?.message}</Text>}
              {!rewardsLoading && !rewardsError && (
                <FlatList
                  data={rewardsCatalog.sort((a, b) => a.cost - b.cost)}
                  keyExtractor={item => `reward-${item.id}`}
                  renderItem={({ item }) => ( <RewardItemStudent item={item} currentBalance={balance} isGoal={item.id === goalRewardId} /> )}
                  ItemSeparatorComponent={() => <View style={{ height: 10 }} />}
          ...
          {activeTab === 'announcements' && (
             <>
              {announcementsLoading && <ActivityIndicator color={colors.primary} style={{ marginVertical: 10 }}/>}
              {announcementsError && <Text style={[appSharedStyles.textDanger, {textAlign: 'center', marginTop: 10}]}>Error loading announcements: {announcementsErrorMsg?.message}</Text>}
              {!announcementsLoading && !announcementsError && (
                <FlatList
                  ...
                  ItemSeparatorComponent={() => <View style={{ height: 10 }} />}
                  ListEmptyComponent={() => ( <Text style={appSharedStyles.emptyListText}>No announcements found.</Text> )}
                  contentContainerStyle={styles.listContentContainer}
                  ListFooterComponent={<View style={{ height: 20 }} />}
                />
               )}
