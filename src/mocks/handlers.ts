...
export const handlers = [
  ...
  http.post('/api/users', async ({ request }) => {
    try {
      const d = (await request.json()) as Omit<User, 'id' | 'status'>;
  ...
  http.delete('/api/users/:id', ({ params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    if (currentMockUsers[id]) {
      delete currentMockUsers[id];
  ...
  http.post('/api/task-library', async ({ request }) => {
    try {
      const d = (await request.json()) as Omit<TaskLibraryItem, 'id'>;
      if (
        !d ||
        !d.title ||
        !d.description ||
        d.baseTickets == null ||
        typeof d.baseTickets !== 'number' ||
        d.baseTickets < 0
      )
        return new HttpResponse('Invalid data', { status: 400 });
      const i = `tasklib-${Date.now()}-${Math.random().toString(36).substring(7)}`;
      const n = { ...d, id: i };
      mockTaskLibraryData.push(n);
      return HttpResponse.json(n, { status: 201 });
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  }),
  http.patch('/api/task-library/:id', async ({ request, params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockTaskLibraryData.findIndex(t => t.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    try {
      const u = (await request.json()) as Partial<Omit<TaskLibraryItem, 'id'>>;
      if (u.baseTickets != null && (typeof u.baseTickets !== 'number' || u.baseTickets < 0))
        return new HttpResponse('Invalid points', { status: 400 });
      const n = { ...mockTaskLibraryData[idx], ...u };
      mockTaskLibraryData[idx] = n;
      return HttpResponse.json(n);
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  }),
  http.delete('/api/task-library/:id', ({ params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockTaskLibraryData.findIndex(t => t.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    mockTaskLibraryData.splice(idx, 1);
    return new HttpResponse(null, { status: 204 });
  }),
  ...
  http.post('/api/rewards', async ({ request }) => {
    try {
      const d = (await request.json()) as Omit<RewardItem, 'id'>;
      if (
        !d ||
        !d.name ||
        d.cost == null ||
        ...
        return new HttpResponse('Invalid data', { status: 400 });
      const i = `reward-${Date.now()}-${Math.random().toString(36).substring(7)}`;
      const n = { ...d, id: i };
      mockRewardsData.push(n);
      return HttpResponse.json(n, { status: 201 });
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  }),
  http.patch('/api/rewards/:id', async ({ request, params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockRewardsData.findIndex(r => r.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    try {
      const u = (await request.json()) as Partial<Omit<RewardItem, 'id'>>;
      if (u.cost != null && (typeof u.cost !== 'number' || u.cost < 0))
        return new HttpResponse('Invalid cost', { status: 400 });
      if (u.imageUrl === '') return new HttpResponse('Image URL empty', { status: 400 });
      const n = { ...mockRewardsData[idx], ...u };
      mockRewardsData[idx] = n;
      return HttpResponse.json(n);
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  }),
  http.delete('/api/rewards/:id', ({ params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockRewardsData.findIndex(r => r.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    mockRewardsData.splice(idx, 1);
    return new HttpResponse(null, { status: 204 });
  }),
  ...
  http.post('/api/announcements', async ({ request }) => {
    try {
      const d = (await request.json()) as Omit<Announcement, 'id' | 'date'>;
      if (!d || !d.title || !d.message || !d.type)
        return new HttpResponse('Invalid data', { status: 400 });
      const i = `ann-${Date.now()}-${Math.random().toString(36).substring(7)}`;
      const n = { ...d, id: i, date: new Date().toISOString() };
      mockAnnouncementsData.unshift(n);
      return HttpResponse.json(n, { status: 201 });
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  ...
  http.patch('/api/announcements/:id', async ({ request, params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockAnnouncementsData.findIndex(a => a.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    try {
      const u = (await request.json()) as Partial<Omit<Announcement, 'id' | 'date'>>;
      const n = { ...mockAnnouncementsData[idx], ...u };
      mockAnnouncementsData[idx] = n;
      return HttpResponse.json(n);
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  ...
  http.get('/api/assigned-tasks', ({ request }) => {
    const url = new URL(request.url);
    const page = parseInt(url.searchParams.get('page') || '1', 10);
    const limit = parseInt(url.searchParams.get('limit') || String(ASSIGNED_TASKS_PAGE_LIMIT), 10);
    const assignmentStatus =
      (url.searchParams.get('assignmentStatus') as TaskAssignmentFilterStatusAPI) || 'all';
    const studentStatus =
  ...
  http.post('/api/assigned-tasks', async ({ request }) => {
    try {
      const d = (await request.json()) as Omit<
        AssignedTask,
      ...
      if (
        ...
        !d.taskTitle ||
        !d.taskDescription ||
        d.taskBasePoints == null ||
        d.assignedById == null
      )
        return new HttpResponse('Invalid data', { status: 400 });
      if (!currentMockUsers[d.studentId])
        return new HttpResponse('Student not found', { status: 404 });
      const i = `assigned-${Date.now()}-${Math.random().toString(16).slice(2)}`;
      const n = {
        ...d,
        id: i,
      ...
      mockAssignedTasksData.push(n);
      return HttpResponse.json(n, { status: 201 });
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  }),
  http.patch('/api/assigned-tasks/:id', async ({ request, params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockAssignedTasksData.findIndex(t => t.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    try {
      ...
      if (u.verificationStatus && e.isComplete && e.verificationStatus === 'pending') {
        const { verificationStatus: vS, verifiedById: vBI, actualPointsAwarded: aPA } = u;
        if (!vBI) return new HttpResponse('Verifier ID required', { status: 400 });
        if (vS !== 'incomplete' && (aPA == null || aPA < 0))
          return new HttpResponse('Valid points required', { status: 400 });
        n.verificationStatus = vS;
        n.verifiedById = vBI;
        n.verifiedDate = new Date().toISOString();
        n.actualPointsAwarded = vS === 'verified' || vS === 'partial' ? aPA : undefined;
        if (n.actualPointsAwarded != null && n.actualPointsAwarded > 0) {
          const sId = n.studentId;
          const t = n.actualPointsAwarded;
          mockTicketBalancesData[sId] = (mockTicketBalancesData[sId] || 0) + t;
          mockTicketHistoryData.unshift({
            id: `tx-${Date.now()}`,
            studentId: sId,
            timestamp: n.verifiedDate,
            amount: t,
            type: 'task_award',
            sourceId: n.id,
            notes: `Task: ${n.taskTitle} (${vS})`,
          });
          console.log(
            `[MSW] Awarded ${t} tickets to ${sId} for task ${id}. Balance: ${mockTicketBalancesData[sId]}`
          );
        }
      }
      mockAssignedTasksData[idx] = n;
      return HttpResponse.json(n);
    } catch (e) {
      return new HttpResponse('Bad request', { status: 400 });
    }
  }),
  http.delete('/api/assigned-tasks/:id', ({ params }) => {
    const { id } = params;
    if (typeof id !== 'string') return new HttpResponse('Invalid ID', { status: 400 });
    const idx = mockAssignedTasksData.findIndex(t => t.id === id);
    if (idx === -1) return new HttpResponse('Not found', { status: 404 });
    mockAssignedTasksData.splice(idx, 1);
    return new HttpResponse(null, { status: 204 });
  }),
  ...
  http.get('/api/ticket-history', ({ request }) => {
    ...
    console.log(
      `[MSW] Responding with ${paginatedItems.length} history items for page ${page}. Total: ${totalItems}`
    );
    return HttpResponse.json({ items: paginatedItems, totalPages, currentPage: page, totalItems });
  }),
  ...
  http.get('/api/students/:id/balance', ({ params }) => {
    ...
    if (
      typeof id !== 'string' ||
      !currentMockUsers[id] ||
      currentMockUsers[id].role !== 'student'
    ) {
      return new HttpResponse('Student not found', { status: 404 });
    }
  ...
  http.post('/api/ticket-adjustments', async ({ request }) => {
    ...
    try {
      const { studentId, amount, notes, adjusterId } = (await request.json()) as {
        studentId: string;
        amount: number;
        notes: string;
        adjusterId: string;
  ...
  http.post('/api/reward-redemptions', async ({ request }) => {
    ...
    try {
      const { studentId, rewardId, redeemerId } = (await request.json()) as {
        studentId: string;
        rewardId: string;
        redeemerId: string;
      };
