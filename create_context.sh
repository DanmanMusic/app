...
# Exclude the output directory itself from the tree command
...
# --- Helper: The Builder Function (Slightly Modified) ---
...
build_context() {
    ...
    # MODIFIED: Run the tree command for the 'docs_and_structure' context
    ...
    printf "%s\n" "${all_files[@]}" | sort -u | while IFS= read -r file; do
        ...
        for excluded in "${GLOBAL_EXCLUDED_FILES[@]}"; do
            if [[ "$file" == "$excluded" ]]; then continue 2; fi
        done
...
# --- Main Execution: The New Simplified Orchestrator ---
...
build_context "context_docs_and_structure.txt" \
    ...
    "package.json" "app.json" "tsconfig.json" "README.md" "TODO.md" "SPECIFICATION.md" "MODEL.md" "ICONS.md"
